...
import channelChartData, {
  fetchChartData,
} from "layouts/dashboards/customer/components/ChannelsChart/data";
import { useBusy } from "layouts/pages/hooks/useBusy";

// ── colour palette (mirrors configs colours) ──────────────────
...
function TotalChart({ id, isAllData, isOpenTicket, startDate, endDate }: ChannelsChartProps): JSX.Element {
      ...
      <Card className="flex h-full flex-col overflow-hidden rounded-2xl border border-border/50 shadow-sm">
        ...
        <CardHeader className="flex flex-row items-center justify-between border-b border-border/40 bg-muted/20 px-4 py-3">
          <CardTitle className="text-sm font-semibold text-foreground">
            Talep Konusu Bazlı Açtığı Talep
          </CardTitle>
          <Button
            type="button"
            variant="ghost"
        ...
        <CardContent className="flex flex-1 flex-col gap-4 p-4">
          ...
          <div className="mt-auto flex flex-col gap-2 border-t border-border/30 pt-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-xs text-muted-foreground leading-relaxed">
              <strong className="font-semibold text-foreground">Talep konusu</strong> bazlı açılan talepleri grafiksel olarak görebilirsiniz.
            </p>
            <Button
              type="button"
              variant="outline"
