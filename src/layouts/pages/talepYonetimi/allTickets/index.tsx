...
const Tip = ({ label, children }: TipProps) => (
...
const EmptyState = () => (
  <div className="flex flex-col items-center justify-center py-20 gap-3">
    ...
    <p className="text-xs text-slate-400">Filtrelerinizi temizlemeyi veya yeni bir talep oluşturmayı deneyin.</p>
  </div>
);

const TicketTable = ({ columns, rows }: TableProps) => {
...
const PageSizeSelect = ({ value, onChange }: PageSizeProps) => (
  <div className="flex items-center gap-2 text-xs text-slate-500">
    ...
    <select
      ...
      className={cn(
        ...
        "outline-none focus:border-slate-400 focus:ring-2 focus:ring-slate-100 transition-all",
        "cursor-pointer"
      )}
    >
...
function AllTickets({ isSolveAllTicket }: AllTicketsProps) {
  ...
  useEffect(() => {
    if (ticketRowData.length === 0) return;
    // Unique value extraction kept for potential filter consumers
    const _departments = [...new Set(ticketRowData.map((t: any) => t.ticketDepartmentText))];
    const _companies = [...new Set(ticketRowData.map((t: any) => t.workCompanyName))];
    const _creators = [...new Set(ticketRowData.map((t: any) => t.userAppName))];
    const _assignees = [...new Set(ticketRowData.map((t: any) => t.ticketAssigneText))];
    void _departments; void _companies; void _creators; void _assignees;
  }, [ticketRowData]);
  ...
  const columns: ColumnDef[] = [
    {
      ...
      className: "w-36 sticky left-0 bg-white group-hover:bg-blue-50/30",
      cell: ({ row }) => {
        const { id, workFlowHeadId, status } = row.original;
        const editUrl = isSolveAllTicket ? "/solveAllTicket/solveTicket" : "/tickets/detail";
        ...
        return (
          <div className="flex items-center gap-0.5">
            ...
            <ActionBtn
              tooltip={t("ns1:TicketPage.TicketTablePage.TalebiDuzenle")}
            ...
            {status === 1 && (
              <ActionBtn
                tooltip={t("ns1:TicketPage.TicketTablePage.TalebiSil")}
                variant="danger"
                onClick={() => { setSelectedTicket(id); setMsgOpen(true); }}
              >
      ...
      <TooltipProvider delayDuration={150}>
        <main className="px-4 md:px-6 py-6 flex flex-col gap-5 min-h-[calc(100vh-10rem)]">
           ...
           <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div>
            <Button size="sm"
              onClick={() => { clearSession(); navigate("/tickets/detail"); }}
              className="mr-4 h-8 gap-2 bg-[#3e5d8f] hover:bg-[#324d7a] text-white font-medium shadow-sm shadow-[#3e5d8f]/25 transition-all duration-200 rounded-lg"
            >
          ...
          <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm shadow-slate-200/50 flex flex-col overflow-hidden">
            ...
            {/* Filter Section */}
            <div className="border-b border-slate-100 flex items-center w-full">
              <FilterTableMethod
