...
const SearchableSelect = <T,>({
  ...
  return (
    <Popover open={open && !disabled} onOpenChange={(o) => !disabled && setOpen(o)}>
      ...
      <PopoverContent className="w-[300px] rounded-xl p-0" align="start">
        <Command>
...
const usersToText = (val: any): string => {
  if (!val) return "";
...
function UserTasks() {
  ...
  const fetchTaskData = async (userId?: string | null) => {
    try {
      ...
      dispatchAlert({ message: "Task verileri getirilirken hata oluştu.", type: "Error" });
    }
  };
  ...
  const exportTasksToExcel = () => {
    ...
    const rows = (taskData ?? []).map((r: any) => ({
      "Atanan Kişiler": usersToText(r.usersFullName),
      Şirket: r.companyName ?? "",
      "Proje Tanımı": r.projectName ?? "",
      "Görev Adı": r.taskName ?? "",
      "Başlangıç Tarihi": r.startDate ? formatDate(r.startDate) : "",
      "Tamamlanma Durumu":
        r.progress !== undefined && r.progress !== null ? `% ${r.progress}` : "",
    }));
    ...
    const colWidths = headers.map((h) => {
      ...
      return { wch: Math.min(60, Math.max(12, h.length, maxCellLen) + 2) };
    });
    ...
    XLSX.writeFile(wb, `Danisman_Gorevleri_${yyyy}-${mm}-${dd}.xlsx`);
  };
    ...
    <DashboardLayout>
      ...
      <div className="flex flex-col gap-6 px-1 py-2">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <p className="text-sm text-muted-foreground">Danışman görevlerini görüntüleyin.</p>
        </div>
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div className="mb-0.5 flex gap-2">
            <Button
              ...
              size="sm"
              disabled={!hasPerm}
              className="gap-1.5 rounded-xl transition-all duration-200 ease-out"
            ...
            <Button
              ...
              className="gap-1.5 rounded-xl bg-emerald-600 text-white transition-all duration-200 ease-out hover:bg-emerald-700 disabled:bg-emerald-600/40"
            ...
            <Table>
              ...
              <TableBody>
                {taskData.length === 0 ? (
                  ...
                  taskData.map((row: any, idx: number) => (
                    <TableRow
                      key={row.id ?? idx}
                      className="border-border/40 transition-colors duration-150"
                    >
                      <TableCell className="px-4 py-3 text-sm">
                        <AssignUsersCell value={row.usersFullName} />
                      </TableCell>
                      <TableCell className="px-4 py-3 text-sm">
                        <GlobalCell value={row.companyName} />
                      </TableCell>
                      <TableCell className="px-4 py-3 text-sm">
                        <GlobalCell value={row.projectName} />
                      </TableCell>
                      <TableCell className="px-4 py-3 text-sm font-medium text-foreground">
                        <GlobalCell value={row.taskName} />
                      </TableCell>
                      <TableCell className="px-4 py-3 text-sm text-muted-foreground">
                        <GlobalCell value={formatDate(row.startDate)} />
                      </TableCell>
                      <TableCell className="px-4 py-3 text-sm">
