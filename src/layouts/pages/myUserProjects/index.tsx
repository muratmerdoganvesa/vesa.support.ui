...
import { WorkCompanyDto } from "api/generated/api";
import { useBusy } from "layouts/pages/hooks/useBusy";

import DashboardLayout from "examples/LayoutContainers/DashboardLayout";
import DashboardNavbar from "examples/Navbars/DashboardNavbar";
import PdfDialog from "../Components/PdfView/PdfDiallog";
import ProjectDashboard from "../projectManagement/projectDashboard/dashboard";
import { fetchMyProjectCompanies } from "./api";
  ...
  useEffect(() => {
    const fetchWorkCompanyData = async () => {
      try {
        ...
        const companies = await fetchMyProjectCompanies();
        setWorkCompanyData(companies);
      } catch (error) {
  ...
  return (
    <DashboardLayout>
      ...
      <div className="relative overflow-hidden rounded-2xl" style={{ height: "80vh" }}>
        ...
        >
          <div className="relative hidden flex-col items-center justify-center overflow-hidden bg-linear-to-br from-indigo-600 via-indigo-700 to-purple-800 p-12 text-white lg:flex lg:w-[42%]">
            ...
            <div className="relative z-10 flex flex-col items-center text-center">
              ...
              <h1 className="mb-3 text-3xl font-bold tracking-tight">Projelerim</h1>
              <p className="max-w-xs text-sm leading-relaxed text-indigo-200">
                Dahil olduğunuz projeleri görüntülemek için şirketinizi seçin.
              </p>
          ...
          <div className="flex flex-1 flex-col items-center justify-center overflow-y-auto bg-background p-8">
            <div className="w-full max-w-sm">
              <div className="mb-6 text-center lg:hidden">
                ...
                <h2 className="text-2xl font-bold tracking-tight">Projelerim</h2>
              ...
              >
                {filteredCompanies.length > 0 ? (
                  ...
                  <div className="py-10 text-center text-sm text-muted-foreground">
                    Dahil olduğunuz proje bulunamadı.
                  </div>
                )}
              </div>
        ...
        >
          {showTest && selectedWorkCompany && (
            <ProjectDashboard
              showTest={showTest}
              selectedWorkCompany={selectedWorkCompany}
              onReturn={handleReturn}
              forCurrentUser
              chartBasePath="/myUserProjects/chart"
              listBadge="Projelerim"
            />
          )}
        </div>
      </div>
